...
public class UserMapperTest {
    ...
    void toUser_fromPrivateDto() {
        UserPrivateDto userPrivateDto = new UserPrivateDto(1L, "username", "email", "password");
    ...
    void toUserPrivateDto() {
        User user = User.builder().id(3L).username("username3").email("email3").password("password3").build();
        ...
        assert userPrivateDto.email().equals("email3");
        assert userPrivateDto.password().equals("password3");
    }

    @Test
    void toUserPublicDto() {
        User user = User.builder().id(4L).username("username4").email("email4").password("password4").build();
